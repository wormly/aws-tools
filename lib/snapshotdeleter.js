...
SnapshotDeleter.prototype._shouldDelete = function(snapshot, day, lastDay) {
	...
	if (age <= DAY_SIZE) {
		console.log("Snapshot", snapshot.Description, snapshot.SnapshotId, "is from the past 24h");
		return false;
	}

	if (age > WEEK_SIZE) {
		console.log("Snapshot", snapshot.Description, snapshot.SnapshotId, "is week or more old");
		return true;
	}

	if (day != lastDay) {
		console.log("Snapshot", snapshot.Description, snapshot.SnapshotId, "is the last of the new day");
		return false;
	}

	console.log("Snapshot", snapshot.Description, snapshot.SnapshotId, "should be deleted");
